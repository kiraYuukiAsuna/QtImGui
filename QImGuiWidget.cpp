...
void QImGuiInterface::impl_keyPressEvent(QKeyEvent *event) {
	if (!event->isAutoRepeat()) {
		...
		if (imgui) {
			...
			if (!event->text().isEmpty()) io.AddInputCharacterUTF16(event->text()[0].unicode());
		}
	}
}
...
void QImGuiWidget::DeleteTexture(GLuint t) {
	if (this->context()->extraFunctions()->glIsTexture(t)) this->context()->extraFunctions()->glDeleteTextures(1, &t);
}
GLuint QImGuiWidget::CreateTexture(const uint8_t *data, int w, int h, int fmt) {
	...
	/*
	当QImGuiWidget窗口作为 QDockWidget 内容时 : QImGuiWidget窗口
	从依附的主窗口挪出时，会造成OpenGl上下文切换 即: this->context()
	所得到的指针会变，原OpenGl上下文
	会被销毁，因此，我们要在原OpenGl上下文销毁前，删除用该
	上下文创造的所有OpenGl资源。方法:"响应 QOpenGLContext::aboutToBeDestroyed
	信号"
	*/
	QObject::connect(this->context(), &QOpenGLContext::aboutToBeDestroyed, [gl, tex]() {
		if (gl->extraFunctions()->glIsTexture(tex)) gl->extraFunctions()->glDeleteTextures(1, &tex);
	});
...
void QImGuiWidget::setFrame(QImGuiInterface *Interface) {
	...
	platform_io.Platform_SetImeDataFn = [](ImGuiContext *ctx, ImGuiViewport *viewport, ImGuiPlatformImeData *data) {
		Q_UNUSED(viewport);
		// 从 context 的 UserData 获取 QImGuiWidget 指针
		if (ctx && ctx->IO.UserData) {
			QImGuiWidget *widget = static_cast<QImGuiWidget *>(ctx->IO.UserData);
			widget->ImeInputPos = QPointF(data->InputPos.x, data->InputPos.y);
			widget->ImeInputLineHeight = data->InputLineHeight;
			// 通知 Qt 更新输入法位置
			QGuiApplication::inputMethod()->update(Qt::ImCursorRectangle);
		}
	};
	// 将 widget 指针存储到 IO.UserData，供回调使用
	ImGui::GetIO().UserData = this;

	Interface->onGlWidgetChanged();
}
void QImGuiWidget::mousePressEvent(QMouseEvent *event) {
	if (FrameInterface) FrameInterface->impl_mousePressEvent(event);
	QOpenGLWidget::mousePressEvent(event);
}
void QImGuiWidget::mouseReleaseEvent(QMouseEvent *event) {
	if (FrameInterface) FrameInterface->impl_mouseReleaseEvent(event);
	QOpenGLWidget::mouseReleaseEvent(event);
}
void QImGuiWidget::mouseDoubleClickEvent(QMouseEvent *event) {
	if (FrameInterface) FrameInterface->impl_mouseDoubleClickEvent(event);
	QOpenGLWidget::mouseDoubleClickEvent(event);
}
void QImGuiWidget::mouseMoveEvent(QMouseEvent *event) {
	if (FrameInterface) FrameInterface->impl_mouseMoveEvent(event);
	QOpenGLWidget::mouseMoveEvent(event);
}
void QImGuiWidget::leaveEvent(QEvent *event) {
	if (FrameInterface) FrameInterface->impl_leaveEvent(event);
	QOpenGLWidget::leaveEvent(event);
}
#if QT_CONFIG(wheelevent)
void QImGuiWidget::wheelEvent(QWheelEvent *event) {
	if (FrameInterface) FrameInterface->impl_wheelEvent(event);
	QOpenGLWidget::wheelEvent(event);
}
#endif
void QImGuiWidget::keyPressEvent(QKeyEvent *event) {
	if (FrameInterface) FrameInterface->impl_keyPressEvent(event);
	QOpenGLWidget::keyPressEvent(event);
}
void QImGuiWidget::keyReleaseEvent(QKeyEvent *event) {
	if (FrameInterface) FrameInterface->impl_keyReleaseEvent(event);
	QOpenGLWidget::keyReleaseEvent(event);
}
void QImGuiWidget::inputMethodEvent(QInputMethodEvent *event) {
	if (FrameInterface) FrameInterface->impl_inputMethodEvent(event);
	QOpenGLWidget::inputMethodEvent(event);
}

QVariant QImGuiWidget::inputMethodQuery(Qt::InputMethodQuery query) const {
	switch (query) {
		case Qt::ImCursorRectangle:
			// 返回输入法候选框应该出现的位置（光标位置）
			return QRectF(ImeInputPos.x(), ImeInputPos.y(), 1, ImeInputLineHeight);
		case Qt::ImEnabled:
			return true;
		default:
...
void QImGuiWidget::paintGL() {
	if (!FrameInterface) return QOpenGLWidget::paintGL();

	// 这里防止的是QT的事件机制导致paintGL
	// 重入,在多个imgui上下文存在时导致一系列的问题
	auto *OldImguiCtx = ImGui::GetCurrentContext();

	// frameDrawing 的存在是为了防止递归调用，RunImguiWidgets
	// 中可能存在阻塞式的QT窗口
	if (!FrameInterface->frame_drawing) {
		...
		for (int i = 0; i < FrameInterface->draw_data.count(); i++) {
			IM_DELETE(FrameInterface->draw_data[i]);
		}
		FrameInterface->draw_data.clear();

		// 克隆新的绘制数据
		for (int i = 0; i < DrawData->CmdListsCount; i++) {
			FrameInterface->draw_data.push_back(DrawData->CmdLists[i]->CloneOutput());
		}
...
void QImGuiWidget::QtOpenGlNewFarme() {
	...
	// 此处是由于 多个widget间共享 FontAtlas, 但 QT opengl
	// 的特性(opengl窗口对象间不共享纹理ID)，因此添加
	if (!FontTex) {
...
void QImGuiWidget::QtOpenGlDevicesClean() {
	...
	if (VaoHandle) funs->glDeleteVertexArrays(1, &VaoHandle);
	if (VaoHandle) funs->glDeleteVertexArrays(1, &VaoHandle);
	if (ElementsHandle) funs->glDeleteBuffers(1, &ElementsHandle);
	if (VboHandle) funs->glDeleteBuffers(1, &VboHandle);
	if (FragHandle) funs->glDeleteShader(FragHandle);
	if (VertHandle) funs->glDeleteShader(VertHandle);
	if (ShaderHandle) funs->glDeleteProgram(ShaderHandle);
	AttribLocationTex = {};
	...
	for (int n = 0; n < FrameInterface->draw_data.count(); n++) {
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
			...
			if (pcmd->UserCallback) {
				...
				if (pcmd->GetTexID() == (ImTextureID)-1)
					funs->glBindTexture(GL_TEXTURE_2D, (GLuint)this->FontTex);
				else
					funs->glBindTexture(GL_TEXTURE_2D, (GLuint)pcmd->GetTexID());
...
// ============== QSimpleImGuiWidget 实现 ==============

class QSimpleImGuiWidget::SimpleInterface : public QImGuiInterface {
public:
	QSimpleImGuiWidget::ImGuiFrameCallback frameCallback;

	SimpleInterface() : QImGuiInterface() {}
	~SimpleInterface() override = default;

	void setCallback(QSimpleImGuiWidget::ImGuiFrameCallback callback) {
...
QSimpleImGuiWidget::QSimpleImGuiWidget(QObject *parent)
	: QImGuiWidget(parent), m_interface(new SimpleInterface()) {
	m_interface->moveToImguiGlWidget(this);
}

QSimpleImGuiWidget::QSimpleImGuiWidget(ImGuiFrameCallback callback, QObject *parent)
	: QImGuiWidget(parent), m_interface(new SimpleInterface()) {
	m_interface->setCallback(std::move(callback));
	m_interface->moveToImguiGlWidget(this);
}

QSimpleImGuiWidget::~QSimpleImGuiWidget() {
	// m_interface 通过 Qt 父子关系自动管理
}

void QSimpleImGuiWidget::setFrameCallback(ImGuiFrameCallback callback) {
	m_interface->setCallback(std::move(callback));
}