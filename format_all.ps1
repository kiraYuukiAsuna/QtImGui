...
# 添加对 ThirdParty 目录的硬编码忽略
$ignorePatterns += "ThirdParty/"
$ignoreRegexList += "^ThirdParty.*"

# 读取 .gitignore 文件并解析出应忽略的文件/目录模式
$gitignorePath = Join-Path $RootDir ".gitignore"

if (Test-Path -Path $gitignorePath) {
    $gitignorePatterns = Get-Content $gitignorePath | Where-Object { $_ -and $_ -notlike '#*' -and $_ -ne '' }
    $ignorePatterns += $gitignorePatterns
    $ignoreRegexList += $gitignorePatterns | ForEach-Object { ConvertGitignorePatternToRegex $_ }
}
